...
class CrossSessionProbe:
    ...
    def run_adaptive(
        ...
        *,
        ...
        """Probe adaptively by expanding from entities observed in prior results."""

        issued: list[str] = []
        queue = list(seed_queries)
        results: list[RetrievalResult] = []
        seen_queries: set[str] = set()
        while queue and len(issued) < query_budget:
            query = queue.pop(0)
            ...
            for edge in result.edges:
                for candidate in _edge_query_terms(edge):
...
def _report_from_results(
    *,
    ...
    per_query_leak_rate = queries_with_leak / len(results) if results else 0.0
    return ProbeReport(
        attacker_session_id=attacker_session_id,
        victim_session_id=victim_session_id,
        query_count=len(results),
...
def _edge_query_terms(edge: MemoryEdge) -> tuple[str, ...]:
    terms = []
    for raw in (edge.source_id, edge.target_id, edge.relation):
        for term in raw.replace("_", "-").split("-"):
            if len(term) >= 4:
                terms.append(term)
