...
- baseline cross-session leaked edges: 3
...
- sessions: 192
- edges: 2304
- black-box baseline leaked edges: 12
- bounded sharing budget 0 leaked edges: 0
- bounded sharing budget 10 utility retention: 0.6141
...
Optional external-corpus run:

```bash
python3 Ideas/GraphMemShield/examples/run_multiwoz_experiment.py
python3 Ideas/GraphMemShield/examples/run_enron_experiment.py
```

Expected key results with the default 1,000-dialogue cap:

- MultiWOZ records: 6613
- MultiWOZ sessions: 955
- MultiWOZ edges: 36408
- baseline leaked edges: 67
- bounded budget 0 leaked edges: 0
- bounded budget 5 leaked edges: 5
- Enron-style records: 160
- Enron-style edges: 447
- Enron-style baseline leaked edges: 3
- Enron-style semantic baseline leaked edges: 2
...
- Edge admission is a seeded one-sided suppression proxy and should not be described as DP.
