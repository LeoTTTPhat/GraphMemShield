...
proxy. We evaluate the framework on a synthetic multi-session graph
and on a Docker-backed mock deployment that ingests a de-identified
JSONL dialogue corpus through a MongoDB-backed cloud API. On the
batched Docker workload (12 sessions, 6 users, 48 edges) the
unmitigated probe exposes \emph{all} 48 victim-owned edges across 48
adversarial queries, while strict provenance/session isolation reduces
exposure to zero. A query-budget analysis demonstrates that a single
entity-anchored probe per session is sufficient to surface every
target edge under the baseline policy, and that allowing \(b{=}1, 2\)
cross-session edges per session pair tightly bounds residual exposure.
We additionally report a \(\varepsilon\)-parameterised sweep of the
write-time admission proxy and discuss how the observed gap between
strict isolation and bounded sharing motivates principled
privacy--utility analysis. We position our results as a reproducible
\emph{benchmark and defensive baseline}---rather than a claim of
end-to-end differential privacy---and outline the additional
adjacency, accounting, and dataset work required to lift the
proxy mechanism to a fully provable guarantee.
\end{abstract}
...
\begin{itemize}
    ...
    \textsc{RandomizedEdgeAdmission}, a seeded write-time admission
    proxy that exposes the privacy--utility surface of a future
    differentially private mechanism.
    \item We release an open-source artefact (Python package, unit
    tests, JSONL ingestion, Docker integration) that reproduces all
    figures and tables in this paper, including a 62-row aggregate
    benchmark across the synthetic and Docker-backed datasets.
    \item We document the framework's deliberate scope: the strict
    \textsc{GraphMemGuard} configuration is reported as a
    \emph{provenance/session isolation baseline}, and the admission
    mechanism is a \emph{seeded proxy} pending formal accounting.
\end{itemize}
...
relation counts, relation bigrams, sensitivity bigrams,
relation co-occurrence pairs, and a degree histogram. The attacker
ranks candidate sessions by cosine similarity to a query session.
Optionally, semantic labels (entity tokens) are added, modelling a
weak pseudonymisation regime. Reported metrics are top-$k$ accuracy
...
$p_\varepsilon = \sigma(\varepsilon)$, where $\sigma$ is the sigmoid.
The decision is seeded by a SHA-256 keyed hash of the edge ID, which
keeps experiments deterministic but is not a substitute for a noise
draw from a formal mechanism. We discuss in
Section~\ref{sec:discussion} the additional adjacency and composition
work required to lift this proxy to a full $\varepsilon$-edge-DP
guarantee.
...
\begin{enumerate}
    \item $S \gets \{ e \in E : \mathrm{match}(e, q) \land \Gamma(e, s_r) \}$
    \item Initialise BFS queue with endpoints of $S$, depth $0$
    \item While queue non-empty and depth $< k$:
    \begin{itemize}
        \item Pop $(v, d)$
        \item For each $e \in \mathrm{adj}(v)$ s.t. $\Gamma(e, s_r)$:
        $S \gets S \cup \{e\}$; enqueue neighbour at $d{+}1$
    \end{itemize}
    \item For $e \in S$: record exposure $\Gamma.\textsc{Record}(e, s_r)$
    \item \textbf{return} $R = (q, s_r, S, \mathrm{nodes}(S))$
\end{enumerate}
\hrule
\label{alg:retrieve}
\end{figure}
...
\noindent\textbf{Synthetic multi-session graph.} A deterministic graph
with two ``Alice'' sessions (medical edges) and one ``Bob'' session
(financial edges). The repeated $(\textit{visited},
\textit{diagnosed})$ motif across Alice's sessions is designed to
expose both cross-session retrieval contamination and structural
linkage.

\noindent\textbf{Docker-backed sample dialogues.} A 12-record
JSONL corpus across 6 users, 12 sessions, and four domains
(\emph{health, finance, work, location}). Records are seeded into a
mock PrivacyGuard MongoDB store and re-fetched through a Cloud REST
API before being mapped into 36 nodes and 48 edges. Sensitivity
labels are propagated through the adapter so that
\textsc{GraphMemGuard} can apply the policy from
Equation~\eqref{eq:guard}.
...
Budgeted runs vary $b \in \{0, 1, 2\}$. Edge-admission runs sweep
$\varepsilon \in \{0.1, 1.0, 3.0\}$.
